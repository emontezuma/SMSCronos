...
Partial Public Class Form1
    ...
    Public Sub New()
        ...
        AddHandler TextEdit19.GotFocus, AddressOf Sombrear_Texto
        AddHandler TextEdit20.GotFocus, AddressOf Sombrear_Texto
        AddHandler TextEdit21.GotFocus, AddressOf Sombrear_Texto
        AddHandler TextEdit22.GotFocus, AddressOf Sombrear_Texto
    End Sub
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim carpeta = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments)

        'HacerLlamada("4422757455", "C:\Users\lap\Documents\Programas\audios2\Audio20180802132555.wav")
  ...
  `z_lla_activar` CHAR(1) DEFAULT NULL,
  `z_lla_veces` INT(3) DEFAULT NULL,
  `z_lla_mail` VARCHAR(100) DEFAULT NULL,
  `z_lla_puerto` VARCHAR(500) DEFAULT NULL,
  `z_sms_comm` VARCHAR(10) DEFAULT NULL,
  `z_lla_sms` CHAR(1) DEFAULT NULL,
  `z_lla_smsvencer` CHAR(1) DEFAULT NULL
        ...
        If CrearScript Then
            Try
                ...
                Dim Comando3 As OdbcCommand = New OdbcCommand("CREATE TABLE `z_mensajes_log` (
  `z_fecha` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'Fecha y hora del registro',
  `z_recipiente` VARCHAR(100) DEFAULT NULL COMMENT 'Recipiente',
  `z_tipo_mensaje` CHAR(1) DEFAULT NULL COMMENT '(S)MS, (E)mail (A)mbos',
  `z_estatus` CHAR(1) DEFAULT NULL COMMENT 'Mensaje enviado (1), No enviado (2)',
  `z_causa` CHAR(1) DEFAULT NULL COMMENT '(1) Recipiente no existe, (2) No hay saldo, (3) Error de autenticación de email (4) Otro error',
  `z_texto` VARCHAR(100) DEFAULT NULL COMMENT 'Texto adicional del error',
  `z_mansaje_enviado` VARCHAR(100) DEFAULT NULL COMMENT 'Mensaje enviado',
  `z_record` DECIMAL(15,0) DEFAULT NULL COMMENT 'ID del registro en mmcall.records',
  `z_message` DECIMAL(15,0) DEFAULT NULL COMMENT 'ID del registro en mmcall.messages'
  ...
  KEY `NewIndex1` (`z_nombre`)
                ...
                Dim Comando5 As OdbcCommand = New OdbcCommand("CREATE TABLE `z_mensajes_recargas` (
  `z_registro` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Fecha y hora del registro',
  `z_fecha` DATE DEFAULT NULL COMMENT 'Fecha de la recarga',
  `z_mensajes` DECIMAL(10,0) DEFAULT NULL COMMENT 'Número de mensajes a recargar',
  `z_reiniciar` CHAR(1) DEFAULT NULL COMMENT 'Reiniciar saldo (S)i (N)o'
) ENGINE=MYISAM DEFAULT CHARSET=latin1 COMMENT='Hostórico de recargas'")
  ...
  `segundos` INT(6) DEFAULT NULL,
  `mensaje` VARCHAR(50) DEFAULT NULL,
  KEY `NewIndex1` (`message`),
  KEY `NewIndex2` (`message`,`record`)
) ENGINE=MYISAM DEFAULT CHARSET=latin1;")
  ...
  `z_frase` VARCHAR(30) DEFAULT NULL COMMENT 'Frase dada por Cronos'
                ...
                Dim ComandoA9 As OdbcCommand = New OdbcCommand("CREATE TABLE `z_mensajes_carpetas` (
  `id` BIGINT(20) NOT NULL AUTO_INCREMENT,
  `area` VARCHAR(100) DEFAULT NULL,
  `carpeta` VARCHAR(500) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `Index01` (`area`)
) ENGINE=MYISAM AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 COMMENT='Rutas para mensajes'
")
                ComandoA9.Connection = MiConexion
                ComandoA9.CommandType = CommandType.Text

                ComandoA9.ExecuteReader()

                Dim ComandoA10 As OdbcCommand = New OdbcCommand("CREATE TABLE `z_mensajes_melodias` (
  `id` BIGINT(20) NOT NULL AUTO_INCREMENT,
  `area` VARCHAR(100) DEFAULT NULL,
  `melodia` VARCHAR(500) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `Index01` (`area`)
) ENGINE=MYISAM AUTO_INCREMENT=3 DEFAULT CHARSET=utf8")
                ComandoA10.Connection = MiConexion
                ComandoA10.CommandType = CommandType.Text

                ComandoA10.ExecuteReader()

                Dim ComandoA11 As OdbcCommand = New OdbcCommand("CREATE TABLE `z_mensajes_traduccion` (
  `id` BIGINT(20) NOT NULL AUTO_INCREMENT,
  `mensaje` VARCHAR(200) DEFAULT NULL,
  `traduccion` VARCHAR(200) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `Index01` (`mensaje`)
) ENGINE=MYISAM AUTO_INCREMENT=21 DEFAULT CHARSET=utf8 COMMENT='Traduccion'")
                ComandoA11.Connection = MiConexion
                ComandoA11.CommandType = CommandType.Text

                ComandoA11.ExecuteReader()
  ...
  `z_frase` varchar(30) DEFAULT NULL COMMENT 'Frase dada por Cronos'
    ...
    Sub RecuperarDatos()
        ...
        Try
            ...
            If Registros.HasRows Then
                ...
                CheckEdit12.Checked = ValNull(Registros!z_lla_activar, "A") = "S"
                CheckEdit13.Checked = ValNull(Registros!z_lla_sms, "A") = "S"
                CheckEdit10.Checked = ValNull(Registros!z_lla_smsvencer, "A") = "S"
                ...
                TextEdit19.Text = Strings.Replace(ValNull(Registros!z_lla_puerto, "A"), "/", "\")
                TextEdit20.Text = ValNull(Registros!z_lla_veces, "N")
                TextEdit21.Text = ValNull(Registros!z_lla_mail, "A")
                TextEdit22.Text = ValNull(Registros!z_sms_comm, "A")
    ...
    Private Sub SimpleButton1_Click(sender As Object, e As EventArgs) Handles SimpleButton1.Click
        ...
        Dim CadSQL As String = "update z_mensajes_config set z_activar = 'S', z_sms_activar = '" & IIf(CheckEdit1.Checked, "S", "N") & "', z_mail_activar  = '" & IIf(CheckEdit2.Checked, "S", "N") & "', z_usar_ssl = '" & IIf(CheckEdit3.Checked, "S", "N") & "', z_rep_activar = '" & IIf(CheckEdit9.Checked, "S", "N") & "', z_aud_activar = '" & IIf(CheckEdit11.Checked, "S", "N") & "', z_lla_activar = '" & IIf(CheckEdit12.Checked, "S", "N") & "', z_lla_sms = '" & IIf(CheckEdit13.Checked, "S", "N") & "', z_lla_smsvencer = '" & IIf(CheckEdit10.Checked, "S", "N") & "', z_rep_ssl = '" & IIf(CheckEdit8.Checked, "S", "N") & "', z_rep_usuario = '" & TextEdit12.Text & "', z_lla_puerto = '" & Strings.Replace(TextEdit19.EditValue, "\", "/") & "', z_rep_contrasena = '" & TextEdit13.Text & "', z_rep_puerto = '" & TextEdit15.Text & "', z_rep_host = '" & TextEdit14.Text & "', z_sms_usuario = '" & TextEdit1.Text & "', z_sms_contrasena = '" & TextEdit2.Text & "', z_sms_manejador = '" & ComboBoxEdit1.SelectedIndex & "', z_aud_genero = " & Val(ComboBoxEdit8.SelectedIndex) & ", z_aud_edad = " & Val(ComboBoxEdit9.SelectedIndex) & ", z_aud_velocidad = " & TrackBarControl1.Value & ", z_aud_pausa = " & Val(ComboBoxEdit7.SelectedIndex) & ", z_aud_guardar = '" & IIf(ComboBoxEdit6.SelectedIndex = 0, "S", "N") & "', z_aud_reproducir = '" & IIf(ComboBoxEdit5.SelectedIndex = 0, "S", "N") & "', z_rep_periodo = '" & ComboBoxEdit2.SelectedIndex & "', z_rep_enviar = '" & ComboBoxEdit3.SelectedIndex & "', z_mail_usuario = '" & TextEdit8.Text & "', z_mail_contrasena = '" & TextEdit7.Text & "', z_mail_puerto = '" & TextEdit5.Text & "', z_mail_host = '" & TextEdit6.Text & "', z_segundos_censo = " & Val(TextEdit9.EditValue) & ", z_lla_veces= " & Val(TextEdit20.EditValue) & ", z_envio = '" & IIf(CheckEdit4.Checked, "S", "N") & "', z_reenvio = '" & IIf(CheckEdit5.Checked, "S", "N") & "', z_lla_mail = '" & TextEdit21.Text & "', z_sms_comm = '" & TextEdit22.Text & "', z_cancela = '" & IIf(CheckEdit6.Checked, "S", "N") & "', z_sms_ssl = '" & IIf(CheckEdit7.Checked, "S", "N") & "', z_sms_host = '" & TextEdit10.EditValue & "', z_rep_subject = '" & Memo1 & "', z_rep_cuerpo = '" & Memo2 & "', z_rep_para = '" & Memo3 & "', z_sms_puerto = '" & TextEdit11.EditValue & "', z_aud_ruta = '" & Strings.Replace(TextEdit16.EditValue, "\", "/") & "', z_aud_ruta2 = '" & Strings.Replace(TextEdit17.EditValue, "\", "/") & "', z_aud_ruta3 = '" & Strings.Replace(TextEdit18.EditValue, "\", "/") & "', z_iniciado = '" & Format(Now, "yyyy/MM/dd HH:mm:ss") & "'"
        Dim Comando As OdbcCommand = New OdbcCommand(CadSQL)
            Comando.Connection = MiConexion
            Comando.CommandType = CommandType.Text
    ...
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        ...
        Dim ELLA As Boolean = False
        Dim ELLS As Boolean = False
        ...
        Else
            ...
            PuertoSerial = ValNull(AppConfig!z_sms_comm, "A").ToString
            ...
            ELLA = ValNull(AppConfig!z_lla_activar, "A").ToString = "S"
            ELLS = ValNull(AppConfig!z_lla_sms, "A").ToString = "S"
            ...
            If Not ESMS And Not EEMA And Not EREP And Not EAUD And Not ELLA And Not ELLS Then
        ...
        End If
        AppConfig.Close()
        If (MEnv Or MRee Or MCan) And (ESMS Or EEMA Or EAUD Or ELLA Or ELLS) Then
            ...
            If Registros.HasRows Then
                ...
                Do While Registros.Read
                    ...
                    'Se valida si el staff es una división o un staff
                    For Each UStaff In ArregloStaff
                        'Se busca el recipiente
                        If UStaff.Trim.Length > 0 Then
                            Dim Comando2 As OdbcCommand = New OdbcCommand("select id from divisions where name = '" & UStaff.Trim & "'")
                            Comando2.Connection = MiConexion
                            Comando2.CommandType = CommandType.Text
                            Recipiente = Comando2.ExecuteReader
                            If Recipiente.HasRows Then
                                Dim MiID As Integer = Recipiente!id
                                Recipiente.Close()
                                Comando2.CommandText = "select name from staffs where division_id = " & MiID
                                Comando2.Connection = MiConexion
                                Comando2.CommandType = CommandType.Text
                                Recipiente = Comando2.ExecuteReader
                                Do While Recipiente.Read
                                    AgregaraArreglo(Recipiente!name)
                                Loop
                                Recipiente.Close()
                            End If
                    ...
                    For Each UStaff In ArregloStaff
                        ...
                        If UStaff.Trim.Length > 0 Then
                            ...
                            If Recipiente.HasRows Then
                                ...
                                End If
                                If ELLA Or ELLS Then
                                    Moviles = Moviles & "," & MMovil
                                End If
                            Else
                    ...
                    If CadToSMS.Length > 0 And ESMS Then
                        ...
                        NoError = EnviarSMSMail(CadStaff, CadToSMS, MPrefijo, AASMSCuenta, AASMSPass, AASSSL, AASHost, AASPuerto, MMensaje, Registros!record_id, Registros!id, MMovil, PuertoSerial)
                        SeEnvio = True
                    End If
                    If EEMA Or ESMS Then
                        ...
                        End If
                    End If
                    Dim GrabarAudio As Boolean = True
                    If TextEdit16.Text.Length = 0 Then GrabarAudio = False
                    ...
                    If EAUD Then
                        Try
                            ...
                            Dim RutaMelodia As String = ""
                            Dim RutaMelodiaUnica As String = ""

                            Dim SinCarpeta As Boolean = False
                            Dim ConPrefijo As Boolean = False
                            Dim AreasconAudio As String()
                            For Each UStaff In ArregloStaff
                                'Se busca el recipiente
                                If UStaff.Trim.Length > 0 Then
                                    'Se busca el prefijo del audio
                                    Comando999.CommandText = "select melodia from z_mensajes_melodias where area = '" & UStaff.Trim & "'"
                                    Comando999.Connection = MiConexion
                                    Traduccion = Comando999.ExecuteReader
                                    If Traduccion.HasRows Then
                                        RutaMelodia = ValNull(Traduccion!melodia, "A")
                                        RutaMelodia = Strings.Replace(RutaMelodia, "/", "\")
                                    End If
                                    If RutaMelodia.Length > 0 Then
                                        If File.Exists(RutaMelodia) Then
                                            ConPrefijo = True
                                        End If
                                    End If
                                    Traduccion.Close()
                                    ...
                                    If RutaAdicional.Length > 0 Then
                                        ...
                                        RutaAudioMerged = RutaAdicional & "\Merged_Audio" & Format(DateAndTime.Now, "yyyyMMddHHmmss") & ".wav"
                                        ...
                                        If ConPrefijo Then
                                            'Se concatenan los dos audios

                                            Dim files() As String = New String() {RutaMelodia, RutaAudio}
                                            Dim wa As WaveIO = New WaveIO
                                            wa.Merge(files, RutaAudioMerged)
                                            'Se borra el mensaje original
                                            File.Delete(RutaAudio)
                                        End If
                                    Else
                                        SinCarpeta = True
                                    End If
                                    If ConPrefijo And RutaMelodiaUnica.Length = 0 And RutaMelodia.Length > 0 Then
                                        RutaMelodiaUnica = RutaMelodia
                            ...
                            If GrabarAudio Then
                                ...
                                RutaAudioMerged = LaRuta & "\Merged_Audio" & Format(DateAndTime.Now, "yyyyMMddHHmmss") & ".wav"
                                Dim synthesizer As New SpeechSynthesizer()
                                ...
                                If ConPrefijo Then
                                    ...
                                    Dim wa As WaveIO = New WaveIO
                                    wa.Merge(files, RutaAudioMerged)
                                    FileCopy(RutaAudioMerged, LaRuta2 & "\Merged_Audio" & Format(DateAndTime.Now, "yyyyMMddHHmmss") & ".wav")
                    ...
                    End If
                    Dim GrabarLlamada As Boolean = True
                    If TextEdit19.Text.Length = 0 Then GrabarLlamada = False

                    '''Llamadas al m´vil
                    If ELLA And Moviles.Length > 0 And GrabarLlamada Then
                        Try
                            'Dim oFileStream, oVoice
                            Dim LaRuta As String
                            If My.Computer.FileSystem.DirectoryExists(TextEdit19.Text) Then
                                LaRuta = TextEdit19.Text
                            Else
                                GrabarLlamada = False
                                LaRuta = Application.StartupPath
                            End If
                            ...
                            End If
                            'Se busca si hay una ruta adicional
                            Dim eMovil As String
                            ArregloMovil = Moviles.Split(New Char() {","c})

                            'Se eliminan los duplicados

                            Dim TempArray As String()
                            For i = LBound(ArregloMovil) To UBound(ArregloMovil)
                                'Redimensionamos el Array temporal y preservamos el valor  
                                ReDim Preserve TempArray(i)
                                'Asignamos al array temporal el valor del otro array  
                                TempArray(i) = ArregloMovil(i)

                            Next

                            For x = 0 To UBound(ArregloMovil)
                                Dim z As Integer = 0

                                For y = 0 To UBound(ArregloMovil)
                                    'Si el elemento del array es igual al array temporal  
                                    If ArregloMovil(x) = TempArray(z) And y <> x Then
                            ...
                            For Each eMovil In ArregloMovil
                                'Se busca el recipiente
                                If eMovil.Length = 10 Then
                                    'Se busca el prefijo del audio
                                    'Comando999.CommandText = "select melodia from z_mensajes_melodias where area = '" & UStaff.Trim & "'"
                                    'Comando999.Connection = MiConexion
                                    'Traduccion = Comando999.ExecuteReader
                                    ...
                                    RutaAudio = LaRuta & "\" & eMovil & Format(DateAndTime.Now, "yyyyMMddHHmmss") & ".wav"
                    ...
                    '''Llamadas al m´vil
                    If ELLS And Moviles.Length > 0 And GrabarLlamada Then
                        Try
                            'Dim oFileStream, oVoice
                            Dim LaRuta As String
                            If My.Computer.FileSystem.DirectoryExists(TextEdit19.Text) Then
                            ...
                            Dim indice = 0
                            For Each eMovil In ArregloMovil
                                If eMovil.Length > 0 Then

                                    indice = indice + 1
                                    'Se busca el recipiente
                                    If eMovil.Length = 10 Then

                                        RutaAudio = LaRuta & "\" & eMovil & Format(DateAndTime.Now, "yyyyMMddHHmmss") & indice & ".txt"

                                        System.IO.File.Create(RutaAudio).Dispose()
                                        Dim objWriter As New System.IO.StreamWriter(RutaAudio, True)
                                        objWriter.WriteLine(MensajeMaster)
                                        objWriter.Close()
                            ...
                            '''
                        Catch ex As Exception
                            Dim Comando99 As OdbcCommand = New OdbcCommand("insert into z_mensajes_log (z_recipiente, z_tipo_mensaje, z_estatus, z_causa, z_texto) values ('Audios', 'A', '2', '1', 'No se pudo generar el archivo de texto para envio de SMS')")
                            Comando99.Connection = MiConexion
                            Comando99.CommandType = CommandType.Text
                            Comando99.ExecuteReader()
                        End Try
                    ...
                    LabelControl36.Text = Registros!id
                    LabelControl36.Refresh()
                        ...
                        If DateAdd("s", MiTiempo, DateTime.Parse(DesdeHora)) <= Now Then
                            ...
                            If Recipiente.HasRows Then
                                ...
                                If MMail.Length > 0 Then
                                    ...
                                    SeEnvio = NoError
                                End If
                                If MMovil.Length > 0 And Val(MMovil) > 0 And ESMS Then
                                    NoError = EnviarSMSMail(CadStaff, MMovil, MPrefijo, AASMSCuenta, AASMSPass, AASSSL, AASHost, AASPuerto, MiMensaje, Registros!record, Registros!message, MMovil, PuertoSerial)
                                    SeEnvio = NoError
                                End If
                                If SeEnvio Then
    ...
    Function EnviarReporte(MMRecipiente As String, MMCuenta As String, MMPrefijo As String, MMUsuario As String, MMPass As String, MMSSL As Boolean, MMHost As String, MMPort As Integer, MMMensaje As String, IDR As Double, IDM As Double, MMSubject As String, FileCSV As String, ElMensaje As String, Hora As String) As Boolean
        ...
        Try
            ...
            Dim Archivo As System.IO.StreamWriter = New System.IO.StreamWriter(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\reportemmcall.csv", False)
            Archivo.WriteLine(FileCSV)
            Archivo.Close()
            Dim at As New Attachment(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\reportemmcall.csv")
            mail.Attachments.Add(at)
            Dim at2 As New Attachment(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\" & NGrafica01)
            mail.Attachments.Add(at2)
            Dim at3 As New Attachment(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\" & NGrafica02)
            mail.Attachments.Add(at3)
            Dim at4 As New Attachment(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\" & NGrafica03)
            mail.Attachments.Add(at4)
            'Dim at5 As New Attachment(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\" & NGrafica04)
            ...
            File.Delete(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\" & NGrafica01)
            File.Delete(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\" & NGrafica02)
            File.Delete(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\" & NGrafica03)
            'File.Delete(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\" & NGrafica04)
            File.Delete(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\reportemmcall.csv")
    ...
    Function EnviarSMSMail(MMRecipiente As String, MMCuenta As String, MMPrefijo As String, MMUsuario As String, MMPass As String, MMSSL As Boolean, MMHost As String, MMPort As Integer, MMMensaje As String, IDR As Double, IDM As Double, Movil As String, PuertoS As String) As Boolean
        ErrorMail = ""
    ...
    Private Sub CheckEdit1_CheckedChanged(sender As Object, e As EventArgs) Handles CheckEdit1.CheckedChanged
        ...
        LabelControl35.Enabled = LabelControl1.Enabled
        ...
        TextEdit22.Enabled = LabelControl1.Enabled
    ...
    Sub Graficas(FechaDesde As String, FechaHasta As String)
        ...
        Try
            ChartControl1.ExportToImage(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\" & NGrafica01, Imaging.ImageFormat.Png)
        Catch ex As Exception
    ...
    Sub Graficas2(FechaDesde As String, FechaHasta As String)
        ...
        Try
            ChartControl1.ExportToImage(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\" & NGrafica02, Imaging.ImageFormat.Png)
        Catch ex As Exception
    ...
    Sub Graficas3(FechaDesde As String, FechaHasta As String)
        ...
        Try
            ChartControl1.ExportToImage(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\" & NGrafica03, Imaging.ImageFormat.Png)
        Catch ex As Exception
    ...
    Sub Graficas4(FechaDesde As String, FechaHasta As String)
        ...
        Try
            ChartControl2.ExportToImage(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\" & NGrafica04, Imaging.ImageFormat.Png)
        Catch ex As Exception
    ...
    Sub AgregaraArreglo(Cadena As String)
        Dim Hallado As Boolean = False

        For Each UStaff In ArregloStaff
            If UStaff.Trim = Cadena.Trim Then
                Hallado = True
                Exit Sub
            End If
        Next
        ReDim Preserve ArregloStaff(ArregloStaff.Count)
        ArregloStaff(ArregloStaff.Count - 1) = Cadena
    End Sub

    Private Sub LabelControl28_Click(sender As Object, e As EventArgs) Handles LabelControl28.Click

    End Sub

    Private Sub HacerLlamada(Numero As String, Archivo As String)
        Try
            SerialPort1.Close()
        Catch ex As Exception
    ...
    Private Sub CheckEdit12_CheckedChanged(sender As Object, e As EventArgs) Handles CheckEdit12.CheckedChanged
        LabelControl31.Enabled = CheckEdit12.Checked Or CheckEdit13.Checked
        TextEdit19.Enabled = CheckEdit12.Checked Or CheckEdit13.Checked
        SimpleButton14.Enabled = CheckEdit12.Checked Or CheckEdit13.Checked
        TextEdit20.Enabled = CheckEdit12.Checked
        LabelControl32.Enabled = CheckEdit12.Checked
        LabelControl34.Enabled = CheckEdit12.Checked
        TextEdit21.Enabled = CheckEdit12.Checked
        CheckEdit10.Enabled = CheckEdit12.Checked
        CambiarBotones(1)
    End Sub

    Private Sub SimpleButton14_Click(sender As Object, e As EventArgs) Handles SimpleButton14.Click
        Procesando = True
        XtraFolderBrowserDialog1.SelectedPath = TextEdit19.Text
        XtraFolderBrowserDialog1.ShowDialog()
        TextEdit19.Text = XtraFolderBrowserDialog1.SelectedPath
        TextEdit19.Focus()
        CambiarBotones(1)
        Procesando = False
    End Sub
    ...
    Private Sub CheckEdit13_CheckedChanged(sender As Object, e As EventArgs) Handles CheckEdit13.CheckedChanged
        LabelControl31.Enabled = CheckEdit13.Checked Or CheckEdit12.Checked
        TextEdit19.Enabled = LabelControl31.Enabled Or CheckEdit12.Checked
        SimpleButton14.Enabled = LabelControl31.Enabled Or CheckEdit12.Checked
